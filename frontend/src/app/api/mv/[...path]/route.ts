...
/**
 * Catch-all proxy for /api/mv/* endpoints
 * This handles all music video API requests that don't have specific proxy routes
 */
...
async function handleRequest(
  ...
  try {
    ...
    const backendUrl = `${BACKEND_URL}/api/mv/${pathString}${queryString ? `?${queryString}` : ''}`
    ...
    if (responseContentType.includes('application/json')) {
      ...
      // Binary/other response (images, videos, etc.)
